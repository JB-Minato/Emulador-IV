...
begin
  ...
  try
    ...
    Logger.Write('LOGIN: 00 07 C7 E3 BC C0 D1 FF',ServerStatus);
    Players.Add(Player);
  finally
    MainCS.Release;
  end;
  ...
  try
    for Player in Players do
      if Player.Socket = Socket then begin
        ...
        Logger.Write('LOGOF: A0 C7 17 C9 CB 00 3D FF',ServerStatus);
  ...
  try
    ...
    for Player in Players do
      if Player.Socket = Socket then begin
        ...
        Logger.Write('LOGIN ERROR: F9 5D AB CC DF A0 3D 00',ServerStatus);
        Players.Remove(Player);
        Player.Free;
      end;
  finally
...
begin
  ...
  Loading.AddHack('Teste.dll');
            //Checagem de CRC Desativada / para ativar remova as tags de comentário!!
  //Loading.AddCheck('mainx.exe','A42B77BEEECE321B19257C7EE803548B9DFD7A42');  // Checagem de CRC do Main
  //Loading.AddCheck('script.kom','5E1959836BDCCE737EA85BCC781B7952274D55B1');  // Checagem de CRC do Script.kom
  Loading.GuildMark:='http://gcreborn.com/GuildMark/';
  Unknown:=TUnknown.Create;
  Players:=TList<TPlayer>.Create;
  MySQL:=TQuery.Create('127.0.0.1',3306,'root','root','gc'); //Aqui você configura as configs da conexão mysql
